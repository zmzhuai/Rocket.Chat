...
async function getApps(instance) {
	...
	let localApp;
	try {
		localApp = (await RocketChat.API.get('apps/')).apps.filter((app) => app.id === id)[0];
		remoteApps = await fetch(`${ HOST }/v1/apps/${ id }?version=${ RocketChat.Info.marketplaceApiVersion }`).then((data) => data.json());
	} catch (error) {
		if (!localApp) {
